...
class Category(models.Model):
    ...
    def get_absolute_url(self):
        return reverse('djobberbase:djobberbase_job_list_category', [self.var_name])
...
class Job(models.Model):
    ...
    if django_version[:2] > (1, 2):
        ...
        category = models.ForeignKey(Category, verbose_name=_('Category'), blank=False, null=False, on_delete=models.SET_NULL)
        jobtype = models.ForeignKey(Type, verbose_name=_('Job Type'), blank=False, null=False, on_delete=models.SET_NULL)
    ...
    def get_edit_url(self):
        return reverse('djobberbase:djobberbase_job_edit', [self.id, self.auth])

    def get_absolute_url(self):
        return reverse('djobberbase:djobberbase_job_detail', [self.id, self.joburl])

    def get_activation_url(self):
        return reverse('djobberbase:djobberbase_job_activate', [self.id, self.auth])

    def get_deactivation_url(self):
        return reverse('djobberbase:djobberbase_job_deactivate', [self.id, self.auth])
